...
module CMS
  module Config
    ...
    class Configuration
      ...
      DEFAULT_CONF = File.join('cms','Config','defaults.yaml')
      ...
      def initialize (hash = Hash.new)
        ...
        raise "Failed to initialize Configuration: Not a hash." unless hash.is_a? Hash

        @source = hash

        # Set values
        @system = CSystem.new(@source['system'])               if @source.key? 'system'
        @page   = CPage.new(@source['page'], @source['options']) if @source.key? 'page'
        @html   = CHTML.new(@source['html'])                   if @source.key? 'html'

      end

      # {{{ Save and Load

      def load (filepath = DEFAULT_CONF)
